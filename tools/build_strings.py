# Turns the locale catalogs into data/strings.js, and keeps them and the two
# frontend files honest.
#
# en.json and nl.json are the maintained sources for user-facing text. Only the
# generated file is packed into the filesystem image, so no catalog occupies
# flash twice and no language is ever fetched.
#
# The checks run with the generation rather than as a separate step, because a
# key a catalog does not answer reaches the user as a fallback word: the page
# still reads as words, but it no longer says what it was written to say.
import json
import os
import re
import sys
...
CATALOGS = {
    "en": os.path.join(REPOSITORY, "en.json"),
    "nl": os.path.join(REPOSITORY, "nl.json"),
}
DEFAULT = "en"
...
STYLES = os.path.join(REPOSITORY, "data", "style.css")
...
def checkStyles(styles, problems):
    # A pseudo-element can put a sentence on the page, and no catalog can reach
    # it there. Decorative content and attr() references carry no words.
    for value in re.findall(r"(?<![-\w])content:\s*([^;}]+)", styles):
        for quoted in re.findall(r"\"([^\"]*)\"|'([^']*)'", value):
            authored = quoted[0] or quoted[1]
            if re.search(r"[A-Za-z]{2,}", authored):
                problems.append(f"data/style.css: text outside the catalog: {authored!r}")


def build():
    catalogs = {name: flatten(json.loads(read(path))) for name, path in CATALOGS.items()}
    reference = catalogs[DEFAULT]
    used = set()
    problems = []
    checkMarkup(read(MARKUP), reference, used, problems)
    checkScript(read(SCRIPT), reference, used, problems)
    checkStyles(read(STYLES), problems)
    for key in sorted(set(reference) - used):
        problems.append(f"{DEFAULT}.json: {key} is not used by the frontend")

    # Every catalog answers the same keys with the same placeholders. Without
    # this a reader in one language meets a key path where a sentence belongs,
    # or a sentence with the address left out of it.
    for name, catalog in catalogs.items():
        if name == DEFAULT:
            continue
        for key in sorted(set(reference) - set(catalog)):
            problems.append(f"{name}.json: {key} is missing")
        for key in sorted(set(catalog) - set(reference)):
            problems.append(f"{name}.json: {key} is not in {DEFAULT}.json")
        for key in sorted(set(reference) & set(catalog)):
            if placeholders(reference[key]) != placeholders(catalog[key]):
                problems.append(f"{name}.json: {key} does not use the same placeholders")

    if problems:
        print("the catalogs and the frontend disagree:", file=sys.stderr)
        for problem in problems:
            print(f"  {problem}", file=sys.stderr)
        sys.exit(1)

    with open(GENERATED, "w", encoding="utf-8", newline="\n") as generated:
        generated.write("// Generated by tools/build_strings.py. Do not edit.\n")
        generated.write("window.strings = ")
        generated.write(json.dumps(catalogs, ensure_ascii=False, separators=(",", ":")))
        generated.write(";\n")
